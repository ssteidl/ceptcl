...
namespace eval ping {    
	...
	set pkt [lindex [read $c] 1]
	::packet::ip_pkt2arr [namespace which -variable in_ippkt] $pkt

    puts [join [::packet::ip_prettypkt $pkt] \n]

	if {[::packet::inet_cksum $pkt] == 0} {
	    puts "ip sum ok"
	} else {
	    puts "ip sum nogood"
	}
	if {[::packet::inet_cksum $in_ippkt(data)] == 0} {
	    puts "icmp sum ok"
	} else {
    ...
    puts [join [::packet::icmp_prettypkt $in_ippkt(data)] \n]
	...
	if ${tr} {
		...
		after 1 [list cepPuts $c $peer $port $ip]
		return
	    }
	} else {
...
#	set ippkt(sourceaddress) 127.0.0.1
	set ippkt(sourceaddress) 192.168.2.2
	set peer [set ippkt(destaddress) $peername]
    }
    ...
    set icmppkt(type) 8
    set icmppkt(code) 0
    set icmppkt(identifier) 6
    set icmppkt(sequence) 7
    set icmppkt(data) [string repeat Boi\x01ng! 10]
    set pkt [::packet::icmp_arr2pkt [namespace which -variable icmppkt]]
    puts [join [::packet::icmp_prettypkt $pkt] \n]
    ...
    puts [join [::packet::ip_prettypkt $ip] \n]
