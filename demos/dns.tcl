...
namespace eval ::dns {
    ...
    if {![catch {package require ceptcl} msg]} { ;# tcludp 1.0.4+
        # If TclUDP 1.0.4 or better is available, use it.
        set options(protocol) udp
    }
...
proc ::dns::resolve {query args} {
    ...
    if {$state(-protocol) == "udp"} {
        if {[package provide ceptcl] == {}} {
            return -code error "udp support is not available, get ceptcl"
        }
    }
...
proc ::dns::UdpTransmit {token} {
    ...
    puts "set state(sock) \[cep -type datagram $state(-nameserver) $state(-port)\]"
    set state(sock) [cep -type datagram $state(-nameserver) $state(-port)]
#    udp_conf $state(sock) $state(-nameserver) $state(-port)
    fconfigure $state(sock) -translation binary -buffering none -blocking 0
    set state(status) connect
